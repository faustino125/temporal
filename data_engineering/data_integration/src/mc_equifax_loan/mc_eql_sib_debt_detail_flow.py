...
def mc_debt_detail_task(
    ...
    """Processes and integrates equifax loan debt detail data

    This task integrates the resulting data from the Data Transformation Layer,
    using features from bi banking.

    Args:
        p_transformed_debt_detail (DataFrame): Transformed  equifax debt detail
        data.
    ...
    window_spec = Window.partitionBy("eqpr_id_cliente_bi", "_observ_end_dt").orderBy(
        f.col("eqpr_fecha_transaccion").desc()
    )
...
@flow(name="mc_eql_sib_debt_detail_flow")
def mc_eql_sib_debt_detail_flow():
    ...
    df_master_debt_detail = mc_debt_detail_task(
        data_transformation["transformed_eql_sib_debt_detail"]
    )
